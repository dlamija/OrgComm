...
<table width="100%" border="0" cellpadding="3" cellspacing="0" bgcolor="#FFFFFF">
    <tr> 
      <td colspan="3" bordercolor="#FF9900"> <strong><font size="2" face="Arial"> Today, <%=CommonFunction.getDate("dd MMM yyyy")%></font></strong>
        <%
	if (conn!=null)
	{
  /*		 String sqlchk2	=  "SELECT am_ref,am_category,am_title,to_char(AM_APPROVE_DATE,'DD MON yyyy'),initcap(sm_staff_name),DM_DEPT_DESC,am_access,ac_cat_desc,am_approve_date,usertype "+
	...
	*/
				String sqlchk2 = "SELECT am_ref,am_category,upper(am_title),to_char(AM_APPROVE_DATE,'DD MON yyyy'), "+
                                 "person_name,am_access,ac_cat_desc,am_approve_date,usertype,am_posted_by "+
                                 "FROM announcement_main,announcement_category,cmsusers_view  "+
                                 "where am_posted_by=cmsid and am_category = ac_cat_id  "+
                                 "and am_official = ? "+
								 "and am_status='APPROVE' "+
								 "and to_char(AM_APPROVE_DATE,'DD MON yyyy')=to_char(sysdate,'DD MON yyyy') "+
                                  "order by AM_APPROVE_DATE DESC ";
   		try
			{
			PreparedStatement pstmtchk2 = conn.prepareStatement(sqlchk2);
			pstmtchk2.setString (1, action);
...
%>
        <% 			if ((type.equals(access)) || (access.equals("Public"))) { %>
    <tr class="contentBgColor"> 
							...
							</font><a href="javascript:void(window.open('entry/view.jsp?ref=<%=rsetchk2.getString(1)%>','view', 'height=700,width=1000,menubar=no,toolbar=no,scrollbars=yes'))"><font size="1" color=blue face="Arial"><b><%=rsetchk2.getString(3)%></b></font></a></font> </font> 
...
<form name="form1" method="post" action="">
  <table width="100%" border="0" cellpadding="3" cellspacing="0" bgcolor="#FFFFFF">
    <tr> 
      <td colspan="3" bordercolor="#FF9900"> <strong><font size="2" face="Arial"> Others</font></strong>
        <%
	if (conn!=null)
	{
  /*		 String sqlchk2	=  "SELECT am_ref,am_category,am_title,to_char(AM_APPROVE_DATE,'DD MON yyyy'),initcap(sm_staff_name),DM_DEPT_DESC,am_access,ac_cat_desc,am_approve_date,usertype "+
	...
	*/
				String sqlchk2 = "SELECT am_ref,am_category,upper(am_title),to_char(AM_APPROVE_DATE,'DD MON yyyy'),person_name,am_access,ac_cat_desc,am_approve_date,usertype,am_posted_by "+
								 ...
								 "where am_posted_by=cmsid and am_category = ac_cat_id "+
								 "and am_official = ? "+
								 "and am_status='APPROVE' and sysdate <= am_approve_date + am_total_day "+
								 "and to_char(sysdate,'DD MON yyyy') <> to_char(AM_APPROVE_DATE,'DD MON yyyy') " +
								 "order by AM_APPROVE_DATE DESC";
   		try
			{
			PreparedStatement pstmtchk2 = conn.prepareStatement(sqlchk2);
