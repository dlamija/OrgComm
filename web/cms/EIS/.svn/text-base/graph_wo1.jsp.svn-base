...
String tahun=year.substring(31);
%><%=tahun%><%
  ...
  <tbody>
    ...
    <tr>
      ...
      <td height="65" valign="top" width="1173"><table width="100%" border="0" cellpadding="3" cellspacing="0">
          ...
          <tr>
            <td width="43%"><span class="style1">Tahun </span></td>
            <td width="5%">:</td>
	...
	String strXML="";
			...
			String sql_graph= " select sm_job_status, count(*) "+
								" from staff_main "+
								" where sm_job_status in ('SAMBILAN', 'TETAP', 'TETAP PERCUBAAN', 'TETAP PENCEN', 'KONTRAK', 'SEMENTARA') "+
								" and to_char(sm_join_date,'YYYY') = '"+tahun+"' "+
								" GROUP BY SM_JOB_STATUS ";
								%><%//=sql_graph%><%
	...
	strXML = "<graph caption='Title: List of Staff Job Status on "+tahun+"' xAxisName='Status' yAxisName='Count' showValues='1' decimalPrecision='0' formatNumberScale='0' showBorder='1'>";
...
if (conn!=null)
   ...
   //String sql_query	=  	"select COUNT (distinct sr_subject_code),sr_subject_code,SM_DESC "+
     //                   "from cmsadmin.subject_registration a, cmsadmin.subject_main b, semester_main c "+
      ...
      String sql_query  = " select sm_job_status, count(*) "+
								" from staff_main "+
								" where sm_job_status in ('SAMBILAN', 'TETAP', 'TETAP PERCUBAAN', 'TETAP PENCEN', 'KONTRAK', 'SEMENTARA') "+
								" and to_char(sm_join_date,'YYYY') = '"+tahun+"' "+
								" GROUP BY SM_JOB_STATUS ";
%><%//=sql_query%><%
        ...
        <TABLE WIDTH="100%" BORDER="1" CELLPADDING="3" CELLSPACING="0" bordercolor="#EAEAEA">
          ...
          <td width="68%" CLASS="contentBgColorAlternate"><strong>List of Job 
            Status</strong></td>
