       ...
       <TD BGCOLOR="#DBDBDB" CLASS="contentBgColor" colspan="7">
         ...
         <TR VALIGN="TOP">
          ...
          if ( (start - limit) > 0)
          {
            ...
            <a href="memo.jsp?action=<%= action %>&folderID=<%= folderID %>&sort=<%= sortBy %>&order=<%= currentSortOrder %>&start=<%= start - limit %>&listing=<%= limit %><%= str %>" onMouseOver="window.status='<%= messages.getString("previous",variable) %>';return true;">
          ...
          <TD BGCOLOR="#DBDBDB" CLASS="contentBgColor" ALIGN="RIGHT">
            <a href="memo.jsp?action=<%= action %>&folderID=<%= folderID %>&sort=<%= sortBy %>&order=<%= currentSortOrder %>&start=<%= start + limit %>&listing=<%= limit %><%= str %>" onMouseOver="window.status='<%= messages.getString("next",variable) %>';return true;">
