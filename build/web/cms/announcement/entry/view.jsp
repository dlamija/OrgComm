...
"sm_student_name,to_char(AM_APPROVE_DATE,'DD MON yyyy'),a.DM_DEPT_DESC,b.dm_dept_desc,eu_name,eu_organization "+
"FROM announcement_main,staff_main,announcement_category,student_main,DEPARTMENT_MAIN a,department_main b,external_user "+
"where am_ref = ? "+
"and am_posted_by=sm_staff_id(+) and ac_cat_id = am_category and sm_student_id(+)=am_posted_by and "+  
"eu_user_id(+) = am_posted_by and SM_DEPT_CODE=a.DM_DEPT_CODE(+) and sm_faculty_code = b.dm_dept_code(+) ";
	...
	try {
		...
		if (rset.next()) {
			...
			external = rset.getString(12);
			organization = rset.getString(13);
...
%>
  ...
  <a href="<%=request.getContextPath()%>/AnnouncementReport?ref=<%=ref%>" onMouseOver="window.status='Print';return true;" target="_blank">
           ...
           <TABLE WIDTH="100%" BORDER="0" CELLSPACING="1" CELLPADDING="3">
            ...
            <tr valign="top" bgcolor="#FFEFCE" class="smallfont"> 
              <td class="contentBgColor"><font size="1" face="Verdana, Arial, Helvetica, sans-serif">Posted 
                by<strong> <%=( ( kiriman ==null)?"":kiriman )%> <%=( ( external ==null)?"":external )%><%=( ( student ==null)?"":student )%></strong>, <%=( ( jabatan ==null)?"":jabatan )%><%=( ( jabatanstd ==null)?"":jabatanstd )%><%=( ( organization ==null)?"":organization )%><br>
