...
<TR VALIGN="MIDDLE">
  <TD BGCOLOR="#EFEFEF" CLASS="contentBgColor" COLSPAN="2">
      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
      <A HREF="calendar.jsp?action=view&date=<%= prevDate %>&month=<%= prevMonth %>&year=<%= prevYear %>" onMouseOver="window.status='<%= messages.getString("calendar.go.back.one.day")  %>';return true;"><IMG SRC="images/system/previous.gif" BORDER="0" ALT="<%= messages.getString("calendar.go.back.one.day")  %>" ALIGN="MIDDLE"></A>
      <%= CommonFunction.parseDate(dateFormat,currentLocale,date+month+year,null,"ddMMyyyy") %><%= today %> 
      <A HREF="calendar.jsp?action=view&date=<%= nextDate %>&month=<%= nextMonth %>&year=<%= nextYear %>" onMouseOver="window.status='<%= messages.getString("calendar.go.forward.one.day")  %>';return true;"><IMG SRC="images/system/next.gif" BORDER="0" ALT="<%= messages.getString("calendar.go.forward.one.day")  %>" ALIGN="MIDDLE"></A>
  </TD>
</TR>
<TR VALIGN="MIDDLE">
...
if (vAppointments != null && vAppointments.size() > 0)
{
  for (i=0; i < vAppointments.size(); i++)
  {
    ...
    <BR>
		...
		{
		  %><a href="servlet/Calendar?action=acceptAppt&userApptID=<%= calendarAppt.getCalendarApptUserID() %>" onMouseOver="window.status='<%= messages.getString("accept") %>';return (true);">
    ...
    if (calendarAppt.getUserID().equals(userID) && eMeetingCode == null)
    {
      ...
      <A HREF="servlet/Calendar?action=deleteAppt&apptID=<%= calendarAppt.getCalendarApptID() %>" ONCLICK="return confirmDelete();" onMouseOver="window.status='<%= messages.getString("delete") %>';return true;">
...
if (vToDo != null && vToDo.size() > 0)
{
  ...
  for(i=0; i < vToDo.size(); i++)
  {
    ...
    if (calendar.getParentUserID().equals(userID) && !emTask.taskLinked(calendar.getCalendarToDoID()))
    {
      ...
      <%= showIcon ? "<IMG SRC=\"images/system/ic_edit.gif\" BORDER=\"0\" ALT=\"Edit\">" : messages.getString("edit") %></A>
      <A HREF="servlet/Calendar?action=deleteToDo&toDoID=<%= calendar.getCalendarToDoID() %>" ONCLICK="return confirmDelete();" onMouseOver="window.status='<%= messages.getString("delete") %>';return true;">
    ...
    if ( calendar.getStatus().equals("0") )
    {
      ...
      %>
      <BR>
      <a href="servlet/Calendar?action=completeToDo&toDoID=<%= calendar.getCalendarToDoID() %>" onMouseOver="window.status='<%= messages.getString("calendar.todo.completed.button") %>';return (true);"><%= showIcon ? "<IMG SRC=\"images/system/ic_completed.gif\" WIDTH=\"57\" HEIGHT=\"18\" BORDER=\"0\" ALT=\"Click here to mark task as completed\">" : messages.getString("calendar.todo.completed.button") %></a>
      <%
      }
    }
		...
		assignedTo =  false;
    if ( calendar.getAllowReassign().equals("1") && calendar.getStatus().equals("0") )
    {
		  temp = "";
      if ( calendar.getParentUserID().equals(userID) ) {
        assignedTo = true;
