...
if (conn!=null)
	...
	String sql =  "SELECT DISTINCT TO_CHAR(SAH.SAH_DATE, 'MM/YYYY') SAH_DATE, TO_CHAR(SAH.SAH_DATE, 'YYYY') SAH_DATE2, TO_CHAR(SAH.SAH_DATE, 'MONTH YYYY') sah_date3 "+
				  "FROM CMSADMIN.STAFF_ATTENDANCE_HEAD SAH,CMSADMIN.STAFF_MAIN SM "+
				  "WHERE SAH.SAH_STAFF_ID = SM.SM_STAFF_ID " +
	        //	  " AND TO_CHAR(SAH_TIME_FROM,'HH24:MI:SS') >='08:01:00'"+
			//	  " and sah_type='ATTENDANCE' "+
				  " AND SAH.SAH_DATE IS NOT NULL "+
				  " AND EXISTS " +
				  "(SELECT 1 FROM CMSADMIN.STAFF_HIERARCHY SH,CMSADMIN.STAFF_ATTENDANCE_HEAD SAHD" +
				  " WHERE SAHD.SAH_STAFF_ID = SH.SH_STAFF_ID " +
				  " AND SH.SH_REPORT_TO='"+sid+"' "+
				  " AND SH.SH_SYS_ID = 'ADM_AL') "+
