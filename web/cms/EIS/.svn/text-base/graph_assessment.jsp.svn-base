...
//String tahun=month.substring(43);
%><%
...
<head>
	...
	function printReport(tahun) 
	{
		if (ReportWin && !ReportWin.closed) ReportWin.close();			
			ReportWin = window.open('printReport.jsp?action=print_assessment&Year='+tahun, 'printReport', 'width=800,height=600,resizable=yes,scrollbars=1');
    ...
    <tr>
      ...
      <td height="65" valign="top" width="1173"><form action="EIS.jsp" method=post>
          ...
          <table width="100%" border="0" cellpadding="3" cellspacing="0">
            <tr> 
              <td width="44%"><div align="right"><span CLASS="contentTitleFont style30 style2"><font size="2">Year</font></span></div></td>
  ...
  <tbody>
   <!--- <tr>
	...
	String strXML="";
			...
			String sql_graph= " SELECT SM_STAFF_ID, SAH_FINAL_MARK "+
							" FROM STAFF_MAIN, STAFF_ASSESSMENT_HEAD "+
							" WHERE SM_STAFF_ID=SAH_STAFF_ID "+
							" AND SAH_FINAL_MARK IS NOT NULL "+
							" and sah_year='"+tahun+"' "+
							" AND SAH_EVALUATOR1='"+sid+"' "+
							" ORDER BY SAH_FINAL_MARK DESC ";
								%><%//=sql_graph%><%
	...
	strXML = "<graph caption='Title: Staff Assessment Mark - "+tahun+"' xAxisName='Staff ID' yAxisName='Percentage ' showValues='1' decimalPrecision='0' formatNumberScale='0' showBorder='1'>";
...
if (conn!=null)
   ...
   //String sql_query	=  	"select COUNT (distinct sr_subject_code),sr_subject_code,SM_DESC "+
     //                   "from cmsadmin.subject_registration a, cmsadmin.subject_main b, semester_main c "+
      ...
      String sql_query  =  " SELECT SM_STAFF_ID, SM_STAFF_NAME, SAH_FINAL_MARK "+
							" FROM STAFF_MAIN, STAFF_ASSESSMENT_HEAD "+
							" WHERE SM_STAFF_ID = SAH_STAFF_ID "+
							" AND SAH_FINAL_MARK IS NOT NULL "+
							" and sah_year = '"+tahun+"' "+
...
%><%//=sql_query%><%
        ...
        <TABLE WIDTH="100%" BORDER="1" CELLPADDING="3" CELLSPACING="0" bordercolor="#EAEAEA">
          ...
          <td width="68%" CLASS="contentBgColorAlternate"><strong>Staff's ID - Staff's Name </strong></td>
            <td width="16%" CLASS="contentBgColorAlternate"><div align="center"><strong>Mark 
              (%) </strong></div></td>
          </tr>
          <%   
		  int jum= 0;
	...
	//	   jum += rset.getInt( 2 );
          ...
          <tr valign="top" bgcolor="#FFFFFF" class="smallfont">
            ...
            <td class = "contentBgColor">&nbsp;</td>
            <td class = "contentBgColor"><div align="center"><strong>&nbsp;</strong></div></td>
          </table>
