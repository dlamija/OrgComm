...
<head>
	...
	function printReport(tahun) 
	{
		if (ReportWin && !ReportWin.closed) ReportWin.close();			
			ReportWin = window.open('printReport.jsp?action=print_leave2&Month='+tahun, 'printReport', 'width=800,height=600,resizable=yes,scrollbars=1');
...
<p align="center">STAFF LEAVE BY MONTH - YEAR</p>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td height="65" valign="top" width="1173"><form action="EIS.jsp" method=post>
          <p>
            <input type="hidden" name="action" value="graph_leave2">
...
if (conn!=null)
	{
	String sql = " SELECT DISTINCT TO_CHAR(SLD_APPLY_DATE, 'MM/YYYY') SLD_APPLY_DATE, TO_CHAR(SLD_APPLY_DATE,'YYYY') SLD_APPLY_DATE2, TO_CHAR(SLD_APPLY_DATE, 'MONTH YYYY') SLD_APPLY_DATE3 "+
				" FROM STAFF_LEAVE_DETL,STAFF_MAIN "+
				" WHERE SLD_STAFF_ID=SM_STAFF_ID "+
				" AND SLD_APPLY_DATE IS NOT NULL "+
				" and sld_status = 'APPROVE' "+
				" and sld_approve_by = '"+sid+"' "+
				//" order by sld_apply_date ";
				"ORDER BY SLD_APPLY_DATE2 desc,SLD_APPLY_DATE desc ";
								%>
                <%//=sql%> 
                <%
		   	try
  ...
  <!---  <tr>
	...
	String strXML="";
			...
			//String sql_graph= " SELECT SM_STAFF_NAME,COUNT (*) "+
				//			" FROM STAFF_LEAVE_DETL,STAFF_MAIN "+
					//		" WHERE SLD_STAFF_ID=SM_STAFF_ID "+
						//	" AND TO_CHAR(SLD_APPLY_DATE, 'MONTH YYYY')= '"+tahun+"' "+
							//" AND SLD_STATUS= 'APPROVE' "+
							//" AND SLD_APPROVE_BY ='"+sid+"' "+
							//" GROUP BY SM_STAFF_NAME ";			
String sql_graph=" SELECT SM_STAFF_ID,COUNT (SLD_TOTAL_DAY) "+
				" FROM STAFF_LEAVE_DETL,STAFF_MAIN "+
				" WHERE SLD_STAFF_ID=SM_STAFF_ID "+
				" and sld_apply_date is not null "+
				" AND TO_CHAR(SLD_APPLY_DATE, 'MONTH YYYY')= '"+tahun+"' "+
	...
	strXML = "<graph caption='TITLE: STAFF LEAVE- "+tahun+"' xAxisName='Staff ID' yAxisName='Count' showValues='1' decimalPrecision='0' formatNumberScale='0' showBorder='1'>";
...
if (conn!=null)
   ...
   //String sql_query	=  	"select COUNT (distinct sr_subject_code),sr_subject_code,SM_DESC "+
     //                   "from cmsadmin.subject_registration a, cmsadmin.subject_main b, semester_main c "+
      ...
      String sql_query  = " SELECT SM_STAFF_ID, SM_STAFF_NAME,COUNT (SLD_TOTAL_DAY) "+
				" FROM STAFF_LEAVE_DETL,STAFF_MAIN "+
				" WHERE SLD_STAFF_ID=SM_STAFF_ID "+
				" and sld_apply_date is not null "+
				" AND TO_CHAR(SLD_APPLY_DATE, 'MONTH YYYY')= '"+tahun+"' "+
