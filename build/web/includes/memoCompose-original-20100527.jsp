...
<table width="100%" border="0" cellpadding="3" cellspacing="1">
  <tr>
    <td><!-- Form for Writing a Memo -->
<form name="composeMemo" action="servlet/Memo?action=compose" method="post" ENCTYPE="multipart/form-data">
<input type="hidden" name="fileList" value=""></td>
<TR VALIGN="TOP" CLASS="contentBgColor"><!--CLASS="contentBgColorAlternate"-->
  <TD COLSPAN="5"><img src="images/hyperlink/memoico.png">
    <B><span class="style13"><%= messages.getString("memo.compose.memo") %></span></B>  </td>
</tr>
...
<TR VALIGN="TOP" BGCOLOR="#DBDBDB">
  <TD CLASS="contentBgColor" bgcolor='#D9F2FF' style="font-family: Verdana, sans-serif; font-size: 11px;  8px; " COLSPAN="4" ALIGN="MIDDLE">




<textarea class="input_text" COLS="40" ROWS="15" id="memoMessage" name="memoMessage" style="width:600; height:250"><%=memoMessage%>
											  </textarea>
...
<TR VALIGN="TOP" BGCOLOR="#DBDBDB">
  <TD CLASS="contentBgColor" bgcolor='#D9F2FF' style="font-family: Verdana, sans-serif; font-size: 11px;  8px; " COLSPAN="3" ALIGN="left">
    <B><%= messages.getString("memo.save.draft") %></B>
	 ...
	 <%
	   if ( TvoContextManager.getSessionAttribute(request,"Memo.saveDraft") != null && TvoContextManager.getSessionAttribute(request,"Memo.saveDraft").equals("1"))
	   {
	 %>
	    <input type="radio" name="saveDraft" value="1" CHECKED>
        <%= messages.getString("yes") %> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <input type="radio" name="saveDraft" value="0">
        <%= messages.getString("no") %>
	 <%
	   }
	   else
	   {
	 %>
        <input type="radio" name="saveDraft" value="1">
        <%= messages.getString("yes") %> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <input type="radio" name="saveDraft" value="0" CHECKED>
        <%= messages.getString("no") %>
	<%
	   }
	%>
...
%>
<TR VALIGN="TOP" BGCOLOR="#DBDBDB">
  <TD CLASS="contentBgColorAlternate" COLSPAN="4" ALIGN="MIDDLE">
  <input type="hidden" name="dummy">
	<A HREF="javascript:checkMemoForm('servlet/Memo?action=compose&type=<%=request.getParameter("type")%>')" onMouseOver="window.status='<%= messages.getString("memo.send") %>';return true;"><%= showIcon  ? "<IMG SRC=\"images/system/ic_send.gif\" BORDER=\"0\" ALT=\"Send\">" :  messages.getString("memo.send") %></A>
<% if (action.equals("draft")) { %>
		<A HREF="javascript:checkMemoForm('servlet/Memo?action=compose&type=<%=request.getParameter("type")%>&memoID=<%= request.getParameter("memoID") %>&isDraftSaved=1')" onMouseOver="window.status='<%= messages.getString("email.draft") %>';return true;"><%= showIcon ? "<IMG SRC=\"images/system/ic_draft.gif\" BORDER=\"0\" ALT=\"Draft\">" : messages.getString("email.draft") %></A>
<% } %>
  <A HREF="memo.jsp?action=folders&folderID=1&type=<%=request.getParameter("type")%>" onMouseOver="window.status='<%= messages.getString("cancel") %>';return true;"><%= showIcon ? "<IMG SRC=\"images/system/ic_cancel.gif\" BORDER=\"0\" ALT=\"Cancel\">" :  messages.getString("cancel") %></A>
  <BR>
  </td>
</tr>
</form>
